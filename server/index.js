...
function normalizeMeliOrder(o) {
    ...
    const sku = items[0]?.sku || 'N/A';
    const ean = items[0]?.ean || 'N/A';
    return { id: `ML-${o.id}`, source: 'Mercado Libre', totalPrice: parseFloat(o.total_amount || 0), createdAt: o.date_created, customer, currency: o.currency_id || 'CLP', totalUnits, sku, ean, items };
}
...
function normalizeRipleyOrder(o) {
    ...
    const items = (o.order_lines || []).map(line => {
        ...
        return { title: normalizedTitle, quantity: line.quantity, price: parseFloat(line.price_unit), sku: codes.sku, ean: codes.ean };
    });
    const sku = items[0]?.sku || 'N/A';
    const ean = items[0]?.ean || 'N/A';
    return { id: `RP-${o.order_id}`, source: 'Ripley', totalPrice: parseFloat(o.total_price || 0), createdAt: o.created_date, customer, currency: 'CLP', totalUnits, sku, ean, items };
}
