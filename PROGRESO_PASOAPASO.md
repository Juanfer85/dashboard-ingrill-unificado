...
194. **[DESPLIEGUE RECLAMOS ML]** Cambios subidos exitosamente a GitHub (`master`) y desplegados en Vercel (`https://dashboard-ingrill-unificado.vercel.app`).
195. **[TIPOGRAFÍA ROBOTO CONDENSED]** Actualizada la tipografía de todo el Dashboard Unificado a **Roboto Condensed** (de Google Fonts). Se importó en `client/index.html`, `client/reporte.html`, `client/informe.html` y sus réplicas en `public/`, aplicando la **Regla de Oro de Sobrescritura Universal** con `!important` para todos los elementos DOM (`body, input, button, select, textarea, th, td, h1-h6, label, span, p, a, div`).
